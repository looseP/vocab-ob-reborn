...
export function verifySingleHostCompose(compose: string, caddyfile: string, environment: string): void {
  ...
  requirePattern(compose, /- "80:80"/, "Public HTTP port");
  requirePattern(compose, /- "443:443"/, "Public HTTPS port");
  ...
  requirePattern(environment, /^APP_ORIGIN=https:\/\//m, "HTTPS origin template");
  requirePattern(environment, /^BACKUP_HOST_DIR=\//m, "Absolute backup directory template");
}
