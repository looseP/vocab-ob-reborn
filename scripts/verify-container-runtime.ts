...
requirePattern(dockerfile, /for name in pg_dump pg_restore; do[\s\S]*?ldd "\$\{binary\}"[\s\S]*?>> \/tmp\/postgres-client-libraries\.raw;[\s\S]*?done;[\s\S]*?sort -u \/tmp\/postgres-client-libraries\.raw > \/tmp\/postgres-client-libraries;/, "Architecture-neutral PostgreSQL client dependency discovery");
requirePattern(dockerfile, /destination="\$\{client_dir\}\/lib\/\$\(basename "\$\{library\}"\)";[\s\S]*?cp -L "\$\{library\}" "\$\{destination\}";/, "Isolated PostgreSQL client dependency copy");
...
requirePattern(dockerfile, /version="\$\(\$\{binary\} --version \| awk 'NR == 1 && \$2 == "\(PostgreSQL\)" \{ print \$3 \}'\)";/, "Actual PostgreSQL client version extraction");
requirePattern(dockerfile, /test "\$\{version\}" = "17\.10";/, "PostgreSQL client version pin validation");
...
requirePattern(dockerfile, /provenance="\$\{client_dir\}\/provenance\.tsv";/, "Backup client provenance output");
requirePattern(dockerfile, /resolved_source="\$\(readlink -f "\$\{source_file\}"\)";/, "Canonical source path resolution");
requirePattern(dockerfile, /dpkg-query -S "\$\{source_file\}" 2>\/dev\/null/, "Debian package owner attribution");
requirePattern(dockerfile, /dpkg-query -S "\$\{resolved_source\}" 2>\/dev\/null/, "Resolved Debian package owner fallback");
requirePattern(dockerfile, /dpkg-query -W -f='\$\{Version\}'/, "Debian package version attribution");
requirePattern(dockerfile, /dpkg-query -W -f='\$\{Architecture\}'/, "Debian package architecture attribution");
requirePattern(dockerfile, /record_file "\$\{binary\}" "\/opt\/postgres-client\/bin\/\$\{name\}\.real";/, "PostgreSQL client binary provenance");
requirePattern(dockerfile, /record_file "\$\{library\}" "\/opt\/postgres-client\/lib\/\$\(basename "\$\{library\}"\)";/, "PostgreSQL client library provenance");
requirePattern(dockerfile, /grep -q '\/opt\/postgres-client\/bin\/pg_dump\.real' "\$\{provenance\}";/, "pg_dump provenance presence");
requirePattern(dockerfile, /grep -q '\/opt\/postgres-client\/bin\/pg_restore\.real' "\$\{provenance\}"/, "pg_restore provenance presence");
