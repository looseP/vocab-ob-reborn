-- 0018: relax the SECURITY DEFINER actor guards on the L2 cache RPCs.
--
-- The original guard required the calling actor to ALREADY have a
-- user_word_l2_progress row for the word — true for the Phase B confirm
-- flow (content confirmed after L2 promotion), but the Phase G candidate
-- pool intentionally supports the "content-first" scenario: agents propose
-- content for words that are not yet in any L2 track (e.g. accent), and the
-- user adopts them BEFORE the word ever joins review. Accepting such a
-- candidate hit "actor cannot refresh L2 cache for word" (42501).
--
-- Relaxation: both RPCs now require an authenticated actor (auth.uid() set
-- through request.jwt.claim.sub) but drop the per-word L2-progress
-- requirement. word_l2_content is word-scoped GLOBAL content (no user_id);
-- the calling routes are owner-only in the operations registry, so the
-- authenticated-actor check keeps the SECURITY DEFINER boundary meaningful.
