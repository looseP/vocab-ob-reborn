...
export function createApp(services: Services, metrics: Telemetry = telemetry): Hono<AppEnv> {
  ...
  // ADR-0037：待录核对面 + 采纳/驳回（owner-only，独立薄路由）。
  app.route("/api/l3", papersAuthoringRoutes(services));
  ...
  // 注册**——静态 /tasks/question-summaries 不得被动态 /tasks/:taskId 吞掉，HTTP 测试兜底）。
  app.route("/api/l3/writing", writingSummariesRoutes(services));
