...
- **Status**: Accepted（v2 增补 2026-09-17：设计卡 v2 裁决，钉增补条 7–12，增补批 T11 随行；验收补记条 13：marks scope 扩 option；条 8 复核修订（2026-09-17）：recheck 随 flags 整段物化；条 14：未作答判据口径统一）
- **Date**: 2026-09-17
...
**14. 未作答判据口径统一（2026-09-17 验收修正，T12b 后修）**：定格软确认与导出/历史摘要的「未作答」判据 = **无作答内容**——`choice`/`choices`/`text` 任一非空才算已作答（单一真源 `hasAnswerContent`）；仅主观痕迹（`marks`/`flags`/`optionFlags`）、空对象、未知形状一律计未答。修正前实现以「answer 非空」判定，导致只标记未选答案的题漏过软确认、导出/历史误标「已作答」。物化不受影响（有 answer 键即物化，痕迹不丢）；draft 痕迹段按**卷面序**渲染（原按 answers 的 jsonb 键序，无语义且序号错乱）。
