# N1 工程验收收尾 · 运行证据与挂起点定位（2026-09-20）
...
- **已定位（挂起点与环境差异）**：挂死机制（vitest `cleanAfterRun → fs.rm(.tmp)` 中一个未完成的 rmdir/unlink 请求 → `await` 永不 resolve → 走不到 `exit()` 兜底）；环境差异的**定量地图**（目录级快/慢分化：快区 20–35ms/轮 vs 慢区 ≈4.1s/轮；慢区偶发永久挂起；最小复现可再现，见 §2.5）。**（2026-09-20 Task07 轮校准：此为「挂起点与环境差异已定位」，不得转述为「根因已闭环」。）**
...
- 挂死入口在**依赖（vitest）的收尾清理**与**本机 fs 行为（按目录位置分化）**的交叉点；**未发现仓库代码可归因缺陷，但系统层根因未闭环**（2026-09-20 Task07 轮校准：原「非仓库代码缺陷」表述改为「挂起点与环境差异已定位；系统层根因未闭环」——挂起点定位 ≠ 系统层根因闭环）；仓库侧无可归因修复点（不改 `node_modules`、不做无依据配置绕行、不降门槛）。
...
- **位置选择（实测依据；全程透明）**：本机 fs 勘察（§2.5）表明「快区」仅两个 Temp 类目录。① 用户 Temp（`os.tmpdir()`）会令 `run-alerting-drill.test.ts` 因「仓库位于 tmpdir 下」而失败（守卫如实拦截 22 例，记录于 §4.1）；② `C:\Windows\Temp` **不在 `os.tmpdir()` 判定内**（守卫原样执行、如实通过），且经压测为快区（29–35ms/轮）。该位置选择属任务书 §三.6「隔离运行路径」范畴（独立 clone、SHA/lock/config 与门禁不变、零代码改动）；此记录供审查复现与复议。
  - **（2026-09-20 Task07 轮校准）**：② 项「不在 os.tmpdir() 判定内」被登记为**守卫遗漏观察项**——运维合同要求演练锁位于所有临时目录之外，而 `scripts/run-alerting-drill.ts` 仅检测 `os.tmpdir()`；该位置下的测试通过部分依赖此检测缺口，**不得作为「门禁绿色」的规范依据，不得被后续批次复用为规避路径**（Task07 轮运行位置改用常规开发目录；见 `study-notes-task07-execution-2026-09-20.md`）。本批不修改告警系统。
- 正式命令（与门禁等值）：
...
- 「慢区」（非 Temp 类目录）为何普遍慢且偶发挂起的**系统层根因未闭环**（§2.4）；工程门禁验收依赖隔离运行路径（§4.2 已完成，原环境挂死不修复）。
- **独立观察项（2026-09-20 登记）：Windows Temp 守卫遗漏**——运维合同要求演练锁在所有临时目录之外；`scripts/run-alerting-drill.ts` 实现仅检测 `os.tmpdir()`（`C:\Users\…\AppData\Local\Temp`），`C:\Windows\Temp` 等其它临时目录不在检测范围。本批不修改告警系统；不得靠该识别缺口制造门禁绿色（详见 `study-notes-task07-execution-2026-09-20.md`）。
