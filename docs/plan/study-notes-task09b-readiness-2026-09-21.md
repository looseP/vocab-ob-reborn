...
- **生命周期关系**（关键约束）：
  - 开关侧栏 → 侧栏容器挂载/卸载，**题纸不动**；
  - 切换笔记（换 `noteId`）→ 编辑器与控制器**重建**（既有语义），但**题纸不动**；
...
| M2 | draft 卷面关闭侧栏再打开 | 无写请求 | 题纸未卸载；无重复 `openSheet` | 题纸数不变 |
...
| M9 | 浏览/搜索/预览/取消后关闭 | **零** `POST` | 全程无创建 | 笔记数不变 |
| M10 | 从当前题发起引用 → 插入 → 保存 → 重开 | `preview` → `PUT`（含 references） | marker + 卡片出现；重开后一致 | `l3_study_note_references` 行存在且 kind 正确 |
| M11 | 保存失败（确定拒绝）→ 修正 → 重试 | 同 requestId 重试 | 可见反馈；重试提交最新内容 | 最终落库为修正后内容 |
| M12 | 未知结果（响应丢失）→ 重试 | 载荷**逐字节相同** | 不重复推进版本 | 版本只 +1 |
| M13 | 双标签 409 | 第二个 `PUT` 返回 409 | 冲突面板；本地保全；仅「复制/显式载入」 | 不自动覆盖服务器行 |
| M14 | IME 组合中触发导航 | 组合期间无写 | 拒答导航；compositionend 后发送完整内容 | 落库为完整内容 |
