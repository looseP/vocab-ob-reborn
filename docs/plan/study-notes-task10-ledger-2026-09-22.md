...
## 8. 合并后变更登记（2026-09-23 补，Task 11 收口轮）

- **侧栏导出出口（P2-1，超范围项补登记）**：实现新增侧栏面板导出按钮（`study-note-panel-export`）与卷面宿主导出动作注册机制，超出任务书 §1 P4 范围且实现期未登记。正式审查（2026-09-22）发现并登记为 P2-1；用户裁决（2026-09-22）：**保留功能**。评估依据与理由见任务书 §7（`study-notes-task10-execution-2026-09-22.md`）。
- 其余 Task 10 内容无合并后变更。
