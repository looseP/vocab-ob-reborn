...
- **CI（只读查询）**：
  - 功能提交 `47b1e2f`/`bb01252`：`Writing E2E` = **completed / success**（已绿）；
  - 台账提交 `2c30ae5`（纯 docs）：一度 pending，随后完成；
  - **最终 head `563946b`：`Writing E2E` = pass（2m4s）**（只读实测，非推测）。
...
## 8. 独立只读复核（完整生命周期，2026-09-21）

不满足于「首次保存后立刻 F5」，另起只读探针在真浏览器 + 隔离 PG 跑完整链路
（`zz-lifecycle.spec.ts`，**复核后已删除，未提交**）：

| 步骤 | 断言 | 实测 |
| --- | --- | --- |
| ① 插入 + 确认 | DB 出现 1 行引用，`captured_at` 有值 | ✅ `2026-09-20T17:25:09.099Z` |
| ② 继续编辑（追加正文） | 引用 **keep**：`captured_at` 与快照不变 | ✅ `same_captured=true` |
| ③ 来源改名+改正文 → 重开 | 卡片「内容已变化」+ liveTitle；快照**仍未**被刷新 | ✅ |
| ④ 转普通摘录 | 消费**已确认服务端快照**（正文含快照文本），引用行归零 | ✅ `body_has_snapshot=true`，refs=0 |
| ⑤ 重开 | 正文与库一致、引用仍为 0 | ✅ |

## 9. 本批结论

- **R1–R4 全部闭环**（先红 → 实现 → 后绿；变异验证有鉴别力且已回退）。
- 分层回归 241/241（前端）+ 356/356（domain/service/api）；E2E **37/37**（原 33 + 新 4）。
- 工程门禁**分段 12 步全 0**；聚合 `test:unit` 因 2 例 `tests/scripts/` 环境性超时退出 1，
  **如实保留**；diff coverage 如实记 N/A。
- 提交链 `47b1e2f`（功能）→ `bb01252`/`2c30ae5`/`563946b`（台账）；
  推送 `1801761..563946b`（普通快进）；PR #129 draft/OPEN/base 未变，
  **最终 head `563946b` 的 Writing E2E = pass**。
- 遗留 2 项（picker「关闭」按钮预存在不可点击；审查探针 2 的整对象 `toEqual` 形状断言）
  已如实登记，均不属 R1–R4 范围。
- **停在 Task 09B 前**：未合并、未 retarget、未部署、未推 main、未开导出/N2。
